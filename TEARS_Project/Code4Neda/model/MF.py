from TEARS_Project.eval_metrics import Recall_at_k_batch, NDCG_binary_at_k_batch, MRR_at_k
from torch.distributions import Normal, kl_divergence, RelaxedOneHotCategorical
import sys 
sys.path.append('../')
from collections import defaultdict

from TEARS_Project.dataloader import map_id_to_genre
...
class otVAE (T5PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = ["decoder.block.0.layer.1.EncDecAttention.relative_attention_bias.weight"]
    _tied_weights_keys = ["encoder.embed_tokens.weight", "decoder.embed_tokens.weight"]
    def __init__(self,config,llm,vae,epsilon,args):
        super().__init__(config)
        self.args =args
    ...
    def generate_recommendations(self,summary,tokenizer,data_tensor,topk,rank = 0 ,alpha = .5,neg = False):
        ...
        okenized_summary = tokenizer([summary],return_tensors="pt")
...
class GenreVae(nn.Module):
    _keys_to_ignore_on_load_unexpected = ["decoder.block.0.layer.1.EncDecAttention.relative_attention_bias.weight"]
    _tied_weights_keys = ["encoder.embed_tokens.weight", "decoder.embed_tokens.weight"]
    def __init__(self,num_genres,prior,genre_map,genres_l,epsilon,concat=False,num_movies = None,args=None):
        super().__init__()
        self.epsilon = epsilon
        self.args =args
        ...
        self.genre_inds = {g.replace('-',' ').lower():i for i,g in enumerate(sorted(genres_l))}
        for g in genre_map:
            for g_ in genre_map[g]:
                self.genre_coutns[g_] += 1
                
        augment_genres = lambda  x : True if args.genre_count  == -1 else (True if x < args.genre_count else False)
        
        self.genre_count_vector = torch.tensor([self.genre_coutns[g] for i,g in enumerate(genres_l) if  augment_genres(i)  ]).float()
        print(f"{self.genre_count_vector.shape=}")
        raise Exception 
    ...
    def generate_recommendations(self,data_tensor,topk,rank = 0 ,alpha = .5,neg = False,small = False, increase = False,nothing= False,mask_genre = None):
        ...
        if mask_genre is not None:
            ...
            elif not small and not neg:


                sentence_rep[:,~mask] = 0
                sentence_rep[:,mask] = 1
            #this is if it is neg
            else:
                sentence_rep[:,mask] = 0
    ...
    def generate_recommendations_batch(self, data_tensor, topk,fav_genre=None, rank=0, small=False, alpha=0.5, neg=False, increase=False, nothing=False, mask_genre=None,proportional = False):
        data_tensor = data_tensor
        # Ensure data_tensor are batched correctly
        if len(data_tensor.shape) == 1:
            data_tensor = data_tensor.unsqueeze(0)
        
        # Generate sentence representations for the batch
        sentence_rep = self.make_genre_vector(data_tensor)
        try:
            vae = self.vae.modules_to_save['default']
        except:
        ...
        if mask_genre is not None :
            ...
            # Get genre indices for each user in the batch
            mask_indices = [self.genre_inds[g] for g in mask_genre]  # List of lists of indices for each user
            fav_genre_indecies = [self.genre_inds[g] for g in fav_genre]
            if  proportional: 
                for i,(mask,fav) in enumerate(zip(mask_indices,fav_genre_indecies)):
                    old_sentence_rep_mask = sentence_rep[i,mask].item()
                    sentence_rep[i,mask] = sentence_rep[i,fav]
                    sentence_rep[i,fav] = old_sentence_rep_mask
            else: 
                for i,(mask,fav) in enumerate(zip(mask_indices,fav_genre_indecies)):

                    sentence_rep[i,mask] = 1
                    sentence_rep[i,~mask] = 0
        ...
        #returned ranked items based on topk
        ranked_logts = torch.topk(logits,topk)
        return [indices.flatten().tolist() for indices in ranked_logts.indices]
       
 
class GenreTEARS(nn.Module):
    _keys_to_ignore_on_load_unexpected = ["decoder.block.0.layer.1.EncDecAttention.relative_attention_bias.weight"]
    _tied_weights_keys = ["encoder.embed_tokens.weight", "decoder.embed_tokens.weight"]
    def __init__(self,num_genres,genre_map,genres_l,epsilon,dropout,concat=False,num_movies = None,args = None):
        super().__init__()
        self.args=args
        self.epsilon = epsilon
        self.num_genres = num_genres
        ...
        self.genre_columns_one_hot = defaultdict(lambda : torch.zeros(num_movies))
        self.num_genres = self.genre_count_vector.shape[0]

        self.classifier = MultiVAE(q_dims = [self.num_genres,800,400],p_dims = [400,num_movies],dropout = dropout)
    ...
    def make_genre_vector(self, input ): 
        ...
        genre_map_tensor = torch.tensor([[int(g in self.genre_map[j]) for i,g in enumerate(sorted(self.genres_l)) if i < self.num_genres] for j in range(input.shape[1])], device=input.device).float()
    ...
    def generate_recommendations(self,data_tensor,topk,rank = 0 ,small = False,alpha = .5,neg = False,increase = False,nothing= False,mask_genre = None,fav_genre= None):
        ...
        return ranked_logts.indices.flatten().tolist()
    def generate_recommendations_batch(self, data_tensor, topk, rank=0, small=False, alpha=0.5, neg=False, increase=False, nothing=False, fav_genre=None,mask_genre=None,proportional = False):
        ...
        # Apply genre masking if specified
        ...
        # Mask out logits for positions in data_tensor that are >= 1
        logits[data_tensor >= 1] = np.log(0.000001)
        
        # Return ranked items based on topk for each item in the batch
        ranked_logits = torch.topk(logits, topk)
        
        return [indices.flatten().tolist() for indices in ranked_logits.indices]
...
class T5Vae(T5PreTrainedModel):
    ...
    def reparameterize(self, mu, logvar):
        if self.training:
            ...
            return mu
    def generate_recommendations_batch(self, summaries, tokenizer, data_tensor, topk, rank=0, alpha=0.5, neg=False, return_emb=False):
        # Ensure data_tensor are batched correctly
        if len(data_tensor.shape) == 1:
            data_tensor = data_tensor.unsqueeze(0)
        # Handle embeddings batch if requested
        if return_emb:
            tokenized_summaries = tokenizer(summaries, return_tensors="pt", padding=True, truncation=True)
            tokenized_summaries = {k: v.to(rank) for k, v in tokenized_summaries.items()}
            return self.forward(data_tensor=data_tensor, input_ids=tokenized_summaries['input_ids'], attention_mask=tokenized_summaries['attention_mask'], alpha=alpha, neg=neg)
        
        # Tokenize the batch of summaries
        ...
        # Move tokenized tensors to the correct device (rank)
        tokenized_summaries = {k: v.to(rank) for k, v in tokenized_summaries.items()}
        ...
        logits = self.forward(
            data_tensor=data_tensor,
            input_ids=tokenized_summaries['input_ids'],
            attention_mask=tokenized_summaries['attention_mask'],
            alpha=alpha,
            neg=neg
        )[0]
    ...
    def generate_recommendations(self,summary,tokenizer,data_tensor,topk,rank = 0 ,alpha = .5,neg = False,return_emb = False,small =False):
        if len(data_tensor.shape) == 1:
            data_tensor = data_tensor.unsqueeze(0)
        ...
        return ranked_logts.indices.flatten().tolist(),logits
...
class sentenceT5Vae(T5PreTrainedModel):
    ...
    def generate_recommendations(self,summary,tokenizer,data_tensor,topk,rank = 0 ,alpha = None ,**kwargs):
        ...
        return ranked_logts.indices.flatten().tolist()
    def generate_recommendations_batch(self, summaries, tokenizer, data_tensor, topk, rank=0, alpha=0.5, neg=False):
        if len(data_tensor.shape) == 1:
            data_tensor = data_tensor.unsqueeze(0)

        # Tokenize summaries as a batch
        tokenized_summaries = tokenizer(summaries, return_tensors="pt", padding=True, truncation=True)

        tokenized_summaries = {k: v.to(rank) for k, v in tokenized_summaries.items()}

        # Perform forward pass with the batch
        semtemce_rep = self.llm_forward(**tokenized_summaries)
        ...
        # Rank and return the top-k indices for each item in the batch
        ranked_logits = torch.topk(logits, topk)

        return [ranked_logits.indices[i].flatten().tolist() for i in range(len(summaries))]
...
class BERTMacridTEARS(nn.Module):
    def __init__(self,
                epsilon,
                args
                ):
        ...
        # Use BERT encoder instead of T5
        ...
        # MLP to project BERT embeddings
        self.mlp = nn.Sequential(
            nn.Linear(self.transformer.config.hidden_size, 800, bias=False),
        )
    ...
    def forward(
        ...
        # Use BERT's pooler output instead of EOS token
        try:
            vae = self.vae.modules_to_save['default']
        except:
            vae = self.vae

        # BERT forward pass with token_type_ids
        bert_outputs = self.transformer(
            input_ids=input_ids,
            attention_mask=attention_mask,
            token_type_ids=None,
            return_dict=True
        ...
        # Use pooled output for sentence representation
        sentence_rep = bert_outputs.pooler_output
        sentence_rep = self.mlp(sentence_rep)
    ...
    def generate_recommendations(self, summary, tokenizer, data_tensor, topk, rank=0, alpha=.5, neg=False, return_emb=False,small=None):
        if len(data_tensor.shape) == 1:
            data_tensor = data_tensor.unsqueeze(0)
            
        if return_emb:
            tokenized_summary = tokenizer(summary, return_tensors="pt", add_special_tokens=True)
            tokenized_summary = {k: v.to(rank) for k, v in tokenized_summary.items()}
            
            return self.forward(
                data_tensor=data_tensor,
                input_ids=tokenized_summary['input_ids'],
                attention_mask=tokenized_summary['attention_mask'],
                alpha=alpha, 
                neg=neg
            )
    ...
    def generate_recommendations_batch(self, summaries, tokenizer, data_tensor, topk, rank=0, alpha=0.5, neg=False, return_emb=False):
        ...
        tokenized_summaries = tokenizer(summaries, return_tensors="pt", padding=True, truncation=True, add_special_tokens=True)
...
class MacridTEARS(T5PreTrainedModel):
    ...
    def forward(
        ...
        if alpha == 1 : 
            ...
            prior_logvar = torch.cat(outputs['logvar_list'],dim=1)
            logits_rec = vae.decode(outputs,z_list = z_rec)['logits']
            logits_merged = logits_rec
            logits_text = logits_rec
    ...
    def generate_recommendations(self,summary,tokenizer,data_tensor,topk,rank = 0 ,alpha = .5,neg = False,small=False,return_emb = False):
        if len(data_tensor.shape) == 1:
            data_tensor = data_tensor.unsqueeze(0)
    ...
    def generate_recommendations_batch(self, summaries, tokenizer, data_tensor, topk, rank=0, alpha=0.5, neg=False,return_emb = False):
        ...
        # Perform forward pass with the batch
        logits = self.forward(data_tensor=data_tensor, 
                            input_ids=tokenized_summaries['input_ids'], 
                            attention_mask=tokenized_summaries['attention_mask'], 
                            alpha=alpha, 
                            neg=neg)[0]
...
class MacridTEARS(T5PreTrainedModel):
    ...
    def forward(
        ...
        if alpha == 1 : 
            ...
            return logits_merged,logits_rec,logits_text,prior_mu,prior_logvar,prior_mu,prior_logvar,z_rec,None
...
def get_tokenizer(args):
    # Check if the BERT tokenizer is specified
    if args.bert:
...
def get_model(args, tokenizer, num_movies, rank, world_size):
    ...
    elif args.embedding_module =='FT5MacridVAE':
...
def get_GenreTEARS(args,num_movies):
    ...
    model = GenreTEARS(num_genres=num_genres,
                       ...
                       num_movies=num_movies,genre_map = id_to_genre,epsilon = args.epsilon,dropout= args.dropout,args=args)
...
def get_t5VAE(args,num_movies):
    ...
    if args.data_name =='ml-1m':
        ...
        p = f'{args.scratch}/saved_model/{args.data_name}/t5_classification_fixed_data_ml-1m_embedding_module_VAE_2024-09-16_20-52-54_2020.csv.pt'
        ...
        p = f'{args.scratch}/saved_model/{args.data_name}/t5_classification_fixed_data_netflix_embedding_module_VAE_2024-09-18_20-15-31_2024.csv.pt'
    elif args.data_name == 'goodbooks':
        p = f'{args.scratch}/saved_model/{args.data_name}/t5_classification_fixed_data_goodbooks_embedding_module_VAE_2024-06-23_18-18-07_2021.csv.pt'
...
def get_t5RecVAE(args,num_movies): 
    ...
    if args.data_name == 'netflix':
        # print('netflix')
        p = f'{args.scratch}/saved_model/{args.data_name}/t5_classification_fixed_data_netflix_embedding_module_RecVAE_2024-09-18_23-50-34_2020.csv.pt'
    elif args.data_name =='ml-1m':
        p = f'{args.scratch}/saved_model/{args.data_name}/t5_classification_fixed_data_ml-1m_embedding_module_RecVAE_2024-09-27_12-37-38_2024.csv.pt'
    elif args.data_name =='goodbooks':
        p = f'{args.scratch}/saved_model/{args.data_name}/t5_classification_fixed_data_goodbooks_embedding_module_RecVAE_2024-06-25_13-09-43_2022.csv.pt'

        
    state_dict = torch.load(p)
    prior.load_state_dict(state_dict,torch.device('cuda:0')
                          )
...
def get_GenreVAE(args,num_movies): 
    ...
    if args.data_name == 'netflix':
        ...
        p = f'{args.scratch}/saved_model/{args.data_name}/t5_classification_fixed_data_ml-1m_embedding_module_RecVAE_2024-09-14_13-53-18_2020.csv.pt'
        # t5_classification_fixed_data_ml-1m_embedding_module_RecVAE_2024-09-13_15-12-55_2024.csv
    ...
    model = GenreVae(num_genres=num_genres, 
                                  ...
                                  genres_l = list(genre_s), epsilon = args.epsilon,concat = args.concat,num_movies = num_movies,args = args)
    # model = get_peft_model(model, lora_config)
    model.set_vae( prior )
    # print(f"{model.vae=}")
    # model.prior.train()
...
def get_MacridTEARS(args,num_movies): 
    ...
    if args.data_name =='ml-1m':
        p = f'{args.scratch}/saved_model/{args.data_name}/t5_classification_fixed_data_ml-1m_embedding_module_MacridVAE_2024-09-16_20-43-29_2023.csv.pt'
    elif args.data_name == 'netflix':
        p = f'{args.scratch}/saved_model/{args.data_name}/t5_classification_fixed_data_netflix_embedding_module_MacridVAE_2024-09-18_20-42-17_2024.csv.pt'
        
    elif args.data_name =='goodbooks':
        
        p = f'{args.scratch}/saved_model/{args.data_name}/t5_classification_fixed_data_goodbooks_embedding_module_MacridVAE_2024-06-26_12-59-01_2022.csv.pt'
    ...
    if args.bert:
        ...
        model = BERTMacridTEARS(args = args, 
...
def get_OT_RecVAE(args,num_movies):
    ...
    prior = MacridVAE(num_movies,args)
    ...
    if args.data_name =='ml-1m':
        ...
        state_dict_llm = torch.load(f'{args.scratch}/saved_model/{args.data_name}/ot_train_vae_ml-1m_embedding_module_VariationalT5_2024-09-18_11-44-02_2020.csv.pt')
    else:
    ...
    llm = get_peft_model(llm, lora_config)
    #turn off the gradient for the prior.q layers
    model = otVAE.from_pretrained('google-t5/t5-base',num_labels=num_movies, classifier_dropout = args.dropout,vae = prior,epsilon = args.epsilon, llm = llm,args = args)
