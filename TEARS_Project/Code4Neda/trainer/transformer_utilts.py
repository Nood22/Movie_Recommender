...
def load_data(args,tokenizer= None, rank=0,world_size=1,prompt_p = None ):
    ...
    if prompt_p is None:
        prompt_p = f"./saved_user_summary/{args.data_name}/user_summary_{args.llm_backbone.replace('/','_')}_.json"
    ...
    with open(prompt_p,'r') as f:
        prompts = json.load(f)    
        prompts = {profile2id[int(float(k))]:v for k,v in prompts.items() if int(float(k)) in profile2id} 
...
def eval_model(args,model, test_dataloader, rank, test_data_tr,precompute_embeddings,loss_f = None, mult_process=True, world_size=2,vae = False,alpha = .5,
    ...
    with torch.no_grad():
        ...
        for b, item in enumerate(test_dataloader):
            ...
            elif args.embedding_module == 'T5Vae' or args.embedding_module =='OTRecVAE' or args.embedding_module =='MacridTEARS' or args.embedding_module =='FT5MacridVAE'  or args.embedding_module =='RecVAEGenreVAE':
            ...
            rolling_loss += loss(movie_emb_clean, labels).item()
            # exit()
...
def train_fineTuning(args, rec_dataloader,augmented_dataloader, model, optimizer, scheduler,pbar,epoch,rank,tokenizer,precompute_embeddings):
    ...
    for b, (items,augmented_items) in enumerate(zip(rec_dataloader,augmented_dataloader)):
        ...
        if precompute_embeddings is not None: 
            ...
            movie_emb,mu,logvar = model(input_ids=input_ids[0].to(rank).unsqueeze(0), attention_mask=attention_mask.to(rank)[0].unsqueeze(0))
            end_time = time.end()
...
def trainT5Vae(args, rec_dataloader, model, optimizer, scheduler,pbar,epoch,rank,precompute_embeddings):
    ...
    if 'RecVAE'  in args.embedding_module:
        ...
        if args.loss == 'MacridTEARSJS_loss':
            
            loss_f = get_loss(args.loss,args)    
...
def parse_args(notebook = False):  # Parse command line arguments
    ...
    parser.add_argument("--llm_backbone", default='gpt4', type=str)
    ...
    parser.add_argument("--turn_off_recsys" ,action='store_true')
    ...
    parser.add_argument('--loss', default='bce_softmax', type=str)
    ...
    parser.add_argument("--genre_count", type=int, default=-1)
